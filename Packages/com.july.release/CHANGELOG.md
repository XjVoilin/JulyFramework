...
- 微信自定义预下载日志收敛为启动标记与结果摘要，保留请求耗时、提交数量和失败原因；移除逐阶段日志、完整 URL 列表及重复请求完成日志。预下载行为和 SDK 日志保持不变。

## 0.4.3 - 2026-09-14

- 抖音改为 JS 并行请求动态预下载清单，在清单与 modulePrepared 均就绪后通过已验证的内部桥接提交一次；请求或提交失败时记录摘要并继续正常启动。
- 移除 game.json 的 preloadDataListUrl，启动前清空静态预下载列表，避免两条路径重复调度；自定义日志收敛为启动标记与结果摘要，下载结果仍由 SDK 日志确认。
- 增加生成脚本的时序、失败和微信回归检查。清单内容、选取预算和微信实现保持不变。
