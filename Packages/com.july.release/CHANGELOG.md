...
- 微信预下载列表请求与引擎启动并行，列表返回后动态提交给 SDK；请求失败或超时不再推迟引擎启动。
- 抖音使用 game.json 的官方 preloadDataListUrl 配置，由平台调度动态预下载；移除等待列表请求完成后才执行 main 的包装。
- 增加带时间戳的清单请求、提交列表、启动入口、SDK 启动阶段与 modulePrepared 日志；抖音实际预下载状态需结合平台 JSFW_PreloadManager 日志确认。
- 保持每个 CoreVersion 一个 preload.json 的发布规则、列表内容与选取预算，不增加 PlanVersion 绑定。
