    ...
    public partial class TDWrapper : TDDynamicSuperPropertiesHandler_PC, TDAutoTrackEventHandler_PC
    {
        ...
        private static void init(TDConfig token)
        {
            ...
            if (!string.IsNullOrEmpty(token.getTimeZoneId()))
            {
                ...
                {
                    UnityEngine.Debug.LogWarning($"[TDPCWrapper] FindSystemTimeZoneById(\"{token.getTimeZoneId()}\") failed: {ex.GetType().Name}: {ex.Message}, using fixed offset fallback");
                    var offset = GetFixedUtcOffset(token.timeZone);
                    if (offset.HasValue)
                    {
                        var id = token.getTimeZoneId();
                        config.SetTimeZone(TimeZoneInfo.CreateCustomTimeZone(id, offset.Value, id, id));
                    }
                }
            }
        ...
        private static TimeSpan? GetFixedUtcOffset(TDTimeZone zone)
        {
            switch (zone)
            {
                case TDTimeZone.UTC:            return TimeSpan.Zero;
                case TDTimeZone.Asia_Shanghai:  return TimeSpan.FromHours(8);
                case TDTimeZone.Asia_Tokyo:     return TimeSpan.FromHours(9);
                case TDTimeZone.America_Los_Angeles: return TimeSpan.FromHours(-8);
                case TDTimeZone.America_New_York:    return TimeSpan.FromHours(-5);
                default: return null;
            }
        }
    }
